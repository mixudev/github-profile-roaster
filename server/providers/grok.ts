...
const rotator = new KeyRotator('GROK_API_KEY');
...
export async function generateWithGrok(
  ...
  if (!rotator.hasKeys) {
    throw new Error('No GROK_API_KEY configured.');
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const err: any = new Error(errBody?.error?.message || `Grok API error: ${res.status}`);
      ...
      if (isRateLimit && rotator.rotate()) {
        console.warn(`[Grok] Key exhausted. Rotating to next key.`);
        continue;
      }
      throw err;
    }
