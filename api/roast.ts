...
// ─── Prompt builder (exact copy of server/lib/promptBuilder.ts) ───────────────
...
7. HEADCANON THE DATA (THE #1 TECHNIQUE FOR ACTUAL LAUGHS — USE THIS A LOT)
   ...
   Good (headcanon): "4 stars total, dan gue berani taruhan 3 di antaranya itu
...
function buildRoastPrompt(
  ...
  const followerRatio = profile.following > 0 ? (profile.followers / profile.following).toFixed(2) : 'N/A';
  ...
  const repoList = repos.length > 0
    ? repos.map(r => `  • "${r.name}" [${r.language || 'No language'}] — "${r.description || 'no description'}" (⭐${r.stargazers_count} stars, 🍴${r.forks_count} forks)`).join('\n')
    : '  (zero public repos — a blank GitHub page staring back like an empty soul)';

  const rawSignals = [
    profile.following > profile.followers * 2 ? `SIGNAL: following=${profile.following}, followers=${profile.followers} (ratio ${followerRatio})` : null,
    zeroStarRepos === repos.length && repos.length > 0 ? `SIGNAL: ${zeroStarRepos}/${repos.length} repos have 0 stars (all of them)` : null,
    tutorialHellCount >= 2 ? `SIGNAL: tutorial-pattern repo names detected — ${[hasTodoApp && 'todo', hasCalculator && 'calculator', hasPortfolio && 'portfolio', hasWeatherApp && 'weather'].filter(Boolean).join(', ')} (${tutorialHellCount} categories)` : null,
    accountAgeYears >= 3 && profile.public_repos < 5 ? `SIGNAL: account_age=${accountAgeYears}y, public_repos=${profile.public_repos} (${(profile.public_repos / accountAgeYears).toFixed(1)} repos/year)` : null,
    profile.bio?.toLowerCase().includes('passion') ? `SIGNAL: bio contains the word "passion"/"passionate"` : null,
    (profile.bio?.toLowerCase().includes('full stack') || profile.bio?.toLowerCase().includes('fullstack')) ? `SIGNAL: bio claims "full stack" — cross-check against languages/repos list` : null,
  ].filter(Boolean).join('\n');
...
4. Is there at least one HEADCANON? → If no, add one.
